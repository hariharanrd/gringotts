...
import {
  ...
  ArrowDownCircle, ArrowUpCircle, CheckCircle2, Clock, StickyNote, Hash, FolderClosed, Landmark
} from 'lucide-react';
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 text-xs font-black uppercase tracking-widest transition-colors border border-slate-200 dark:border-slate-700/50"
          >
            <Pencil className="w-3.5 h-3.5" /> Edit
          </button>
          <button
            onClick={() => setConfirmDelete(true)}
            className="flex items-center gap-2 px-4 py-2 rounded-xl bg-rose-50 dark:bg-rose-900/20 hover:bg-rose-100 dark:hover:bg-rose-900/40 text-rose-600 dark:text-rose-400 text-xs font-black uppercase tracking-widest transition-colors border border-rose-200 dark:border-rose-800/50"
          >
      ...
      <div className={`rounded-[2rem] bg-gradient-to-br ${config.gradient} p-6 text-white shadow-lg relative overflow-hidden group`}>
        ...
        <div className="absolute top-0 right-0 p-6 opacity-10 group-hover:scale-110 transition-transform duration-700">
          <TypeIcon className="w-32 h-32 -mr-8 -mt-8" />
        </div>
        
        <div className="relative z-10 space-y-6">
          <div className="flex flex-wrap items-center gap-2.5">
            <span className="px-2.5 py-1 bg-white/10 backdrop-blur-md rounded-full text-[9px] font-black uppercase tracking-widest border border-white/10">
              {type}
            </span>
            <span className="px-2.5 py-1 bg-white/10 backdrop-blur-md rounded-full text-[9px] font-black uppercase tracking-widest border border-white/10 tabular-nums">
              #{transaction.id}
            </span>
          </div>

          <div className="space-y-1">
            <p className="text-white/70 font-medium text-sm ml-0.5 line-clamp-1">{transaction.description}</p>
            <div className="text-4xl font-black tabular-nums flex items-baseline gap-1.5">
      ...
      <div className="glass-card p-6 md:p-8 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700/50">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-10 gap-y-6">
          ...
          {/* Funding Loan Info */}
          {transaction.funding_loan && (
            <Field icon={Landmark} label="Funded from Loan"
              value={
                <button
                ...
                >
                  <span>🏛️ {transaction.funding_loan.name}</span>
                </button>
              }
            />
          )}
          ...
          {transaction.schedule_id && (
            <Field icon={RefreshCw} label="Created Via"
              value={
                <button
                  onClick={() => navigate(`/schedules/${transaction.schedule_id}`)}
                  className="flex items-center gap-1.5 px-3 py-1 bg-cyan-50 dark:bg-cyan-900/30 text-cyan-600 dark:text-cyan-400 rounded-full text-xs font-bold hover:bg-cyan-100 dark:hover:bg-cyan-900/50 transition-all border border-cyan-100 dark:border-cyan-800"
                >
