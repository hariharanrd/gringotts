...
import { Plus, Landmark, TrendingUp, Edit2, Trash2, CheckCircle2, ChevronRight, ChevronDown, Sparkles, HelpCircle, Receipt } from 'lucide-react';
...
const Loans: React.FC = () => {
  ...
  const closedLoans = loans.filter(l => l.is_closed);
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="space-y-8">
              ...
              {activeLoans.map(loan => {
                ...
                return (
                  <div
                    ...
                    className="group relative bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-3xl border border-slate-200 dark:border-slate-800/80 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-black/5 cursor-pointer overflow-hidden"
                  >
                    ...
                    <div className="p-5 space-y-4">
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex items-center gap-3">
                          <div className="w-11 h-11 rounded-2xl flex items-center justify-center text-xl shrink-0 shadow-sm bg-cyan-500/10 text-cyan-600 dark:text-cyan-400">
          ...
          {/* Closed Loans Collapsible Section */}
          {closedLoans.length > 0 && (
            <div className="space-y-4">
              {/* Section Toggle Header */}
              <button
                onClick={() => setClosedExpanded(prev => !prev)}
                className="w-full flex items-center justify-between px-4 py-3 rounded-2xl bg-slate-100/80 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/60 hover:bg-slate-200/60 dark:hover:bg-slate-800 transition-all group"
              >
                <div className="flex items-center gap-3">
                  <div className="w-7 h-7 rounded-xl bg-slate-300/60 dark:bg-slate-700 flex items-center justify-center">
                    <Landmark className="w-3.5 h-3.5 text-slate-500 dark:text-slate-400" />
                  </div>
                  <div className="text-left">
                    <span className="text-sm font-bold text-slate-600 dark:text-slate-300">Closed Loans</span>
                    <span className="ml-2 text-[10px] font-semibold text-slate-400 dark:text-slate-500 bg-slate-200 dark:bg-slate-700 px-2 py-0.5 rounded-full">
                      {closedLoans.length}
                    </span>
                  </div>
                </div>
                <ChevronDown
                  className={`w-4 h-4 text-slate-400 transition-transform duration-300 ${closedExpanded ? 'rotate-180' : ''}`}
                />
              </button>
                  ...
                  {closedLoans.map(loan => {
                    ...
                    return (
                      <div
                        key={loan.id}
                        onClick={() => handleCardClick(loan)}
                        className="group relative bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl rounded-3xl border border-slate-200/60 dark:border-slate-800/50 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:shadow-black/5 cursor-pointer overflow-hidden opacity-75 hover:opacity-100"
                      >
                        {/* Visual Top Highlight Accent */}
                        <div className="h-1.5 w-full bg-slate-400 dark:bg-slate-700" />
                        ...
                        <div className="p-5 space-y-4">
                          ...
                          <div className="flex items-start justify-between gap-3">
                            <div className="flex items-center gap-3">
                              <div className="w-11 h-11 rounded-2xl flex items-center justify-center text-xl shrink-0 shadow-sm bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500">
                                <Landmark className="w-5 h-5" />
                              </div>
                              <div>
                                <h3 className="font-extrabold text-slate-700 dark:text-slate-300 text-sm leading-tight line-clamp-1">
                                  {loan.name}
                                </h3>
                                <p className="text-[10px] text-slate-400 dark:text-slate-500 font-semibold uppercase tracking-wider mt-0.5">
                                  {loan.lender || 'Personal Loan'}
                          ...
                          <div className="grid grid-cols-2 gap-2 bg-slate-50 dark:bg-slate-950 p-3 rounded-2xl border border-slate-150 dark:border-slate-850">
                            <div>
                              <span className="text-[9px] uppercase font-bold text-slate-400">Monthly EMI</span>
                              <p className="text-sm font-extrabold text-slate-500 tabular-nums">{fmt(loan.emi_amount)}</p>
                            </div>
                            <div>
                              <span className="text-[9px] uppercase font-bold text-slate-400">Outstanding Balance</span>
                              <p className="text-sm font-extrabold text-slate-600 dark:text-slate-400 tabular-nums">{fmtCompact(sum.outstanding_principal)}</p>
                            </div>
                          </div>
                          ...
                          <div className="space-y-1.5">
                            ...
                            <div className="w-full bg-slate-100 dark:bg-slate-950 rounded-full h-2.5 overflow-hidden p-0.5 border border-slate-200 dark:border-slate-850">
                              <div
                                className="h-1.5 rounded-full bg-gradient-to-r from-slate-400 to-slate-500 transition-all duration-550 shadow-sm"
                                style={{ width: `${pct}%` }}
                              />
                            </div>
                          </div>
                          ...
                          {/* Settled badge */}
                          <div className="py-1.5 text-center bg-slate-100 dark:bg-slate-850 text-slate-500 dark:text-slate-450 font-bold rounded-xl text-[10px] uppercase tracking-wider">
                            ✓ Balance Settled
                          </div>
