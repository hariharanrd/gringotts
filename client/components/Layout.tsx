...
const Layout: React.FC<LayoutProps> = ({ userName, children, onImport }) => {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-slate-950">
      ...
      {/* Sidebar */}
      <aside className={`
        fixed inset-y-0 left-0 z-50 w-72 bg-slate-900/80 backdrop-blur-2xl border-r border-slate-700/50 transform transition-transform duration-300 ease-out
      ...
      `}>
        <div className="flex flex-col h-full">
          ...
          <div className="p-6 flex items-center gap-3 border-b border-slate-700/50">
            ...
            <div>
              <h1 className="text-lg font-bold text-white tracking-tight">Gringotts</h1>
              <p className="text-[10px] text-slate-500 uppercase tracking-widest font-medium">Finance Tracker</p>
            </div>
          </div>
          ...
          <div className="p-4 border-t border-slate-700/50">
            <div className="flex items-center gap-3 px-3 py-2">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[11px] text-slate-500">Vault Keeper</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <main className="flex-1 overflow-y-auto bg-slate-950 relative">
        {/* Header */}
        <header className="sticky top-0 z-30 bg-slate-950/80 backdrop-blur-xl border-b border-slate-800/60 px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 relative">
            {onImport && (
              <button
                onClick={onImport}
                className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-slate-300 bg-slate-800/60 border border-slate-700/50 rounded-xl hover:bg-slate-700/60 hover:text-white transition-all"
              >
            ...
            <button 
              ...
              className="flex items-center gap-3 hover:bg-slate-800/60 p-2 pr-4 rounded-xl transition-all duration-200"
            >
            ...
            {isProfileOpen && (
              <div className="absolute top-full right-0 mt-2 w-48 glass rounded-xl shadow-2xl shadow-black/40 py-2 z-50">
                <Link 
                  to="/logout" 
                  className="flex items-center gap-3 px-4 py-2.5 text-sm text-rose-400 hover:bg-rose-500/10 transition-colors w-full font-medium"
