...
const Toast: React.FC<ToastProps> = ({ message, type, onClose, duration = 3000 }) => {
  ...
  const styles = type === 'success' 
    ? 'border-emerald-500/30 bg-slate-900/95 shadow-emerald-500/10' 
    : 'border-rose-500/30 bg-slate-900/95 shadow-rose-500/10';
  
  const iconColor = type === 'success' ? 'text-emerald-400' : 'text-rose-400';
  const textColor = type === 'success' ? 'text-emerald-100' : 'text-rose-100';
